...
void IMU_Unlock_Instruct(void)
{
	...
	for (int i = 0; i < 8; i++)
	{
		HAL_UART_Transmit_DMA(&huart_605, (unlock_buffer + i),1);
	}
}
...
void IMU_Save_Instruct(void)
{
	...
	for (int i = 0; i < 8; i++)
	{
		HAL_UART_Transmit_DMA(&huart_605, (save_buffer + i),1);
	}
}

uint16_t IMU_Request_Data(uint16_t ADDR, uint16_t LEN)
{
	//	__HAL_UART_DISABLE_IT(&huart_605,UART_IT_RXNE);
...
//		USART1->DR = (uint8_t)(*(request_buffer + i)); /* 将要发送的字符写入到DR寄存器 */
		HAL_UART_Transmit_DMA(&huart_605,request_buffer,8);
	//	__HAL_UART_ENABLE_IT(&huart_605,UART_IT_RXNE);
