...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
		Data_Analyse(ops9_data_buffer[0]);
	}
...
void Data_Analyse(uint8_t rec)
{
	...
	switch(count)
	{
		...
		case 4:
			if(ch==0x0d)
			{
				ops9_data.Euler.yaw=posture.ActVal[0];   		//zangle
				ops9_data.Euler.pitch=posture.ActVal[1];			//xangle
				ops9_data.Euler.roll=posture.ActVal[2];			//yangle
				ops9_data.Position.pos_x=posture.ActVal[3];
				ops9_data.Position.pos_y=posture.ActVal[4];
				ops9_data.Euler_velocity.yaw_velocity=posture.ActVal[5];
			}
			count=0;
			break;
		default:
